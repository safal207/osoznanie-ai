...
def _normalize_ids(values: list[str], *, field_name: str, required: bool) -> list[str]:
    normalized = sorted({value.strip() for value in values if value.strip()})
    if required and not normalized:
        raise ValueError(f"{field_name} must contain at least one non-blank ID")
    return normalized
...
class SuccessCriterion(CanonicalProtocolRecord):
    ...
    @field_validator("fixed_at")
    @classmethod
    def normalize_fixed_at(cls, value: datetime) -> datetime:
        return _require_aware(value, field_name="fixed_at")

    @model_validator(mode="after")
    def validate_operator_value(self) -> SuccessCriterion:
        if self.operator in {
            CriterionOperator.GT,
            CriterionOperator.GTE,
            CriterionOperator.LT,
            CriterionOperator.LTE,
        } and isinstance(self.expected_value, (str, bool)):
            raise ValueError("ordered comparison operators require a numeric expected value")
        return self
...
class LessonApplication(CanonicalProtocolRecord):
    ...
    def normalize_applied_at(cls, value: datetime) -> datetime:
        return _require_aware(value, field_name="applied_at")
...
class OutcomeObservation(CanonicalProtocolRecord):
    ...
    @field_validator("lesson_application_ids")
    @classmethod
    def normalize_application_ids(cls, values: list[str]) -> list[str]:
        return _normalize_ids(
            values,
            field_name="lesson_application_ids",
            required=True,
        )

    @field_validator("source_event_ids", "evidence_ids")
    @classmethod
    def normalize_optional_references(cls, values: list[str]) -> list[str]:
        return _normalize_ids(values, field_name="references", required=False)
...
class CriterionEvaluation(CanonicalProtocolRecord):
    ...
    def normalize_observation_ids(cls, values: list[str]) -> list[str]:
        return _normalize_ids(values, field_name="observation_ids", required=False)
    ...
    def normalize_evaluated_at(cls, value: datetime) -> datetime:
        return _require_aware(value, field_name="evaluated_at")
