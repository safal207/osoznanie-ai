...
from .application import CriterionEvaluation, LessonApplication, OutcomeObservation, SuccessCriterion
...
def _contract_only(value: Any) -> Any:
    if isinstance(value, dict):
        return {key: _contract_only(item) for key, item in value.items() if key != "title"}
...
def render_schema(record_type: str, model: type[BaseModel]) -> str:
    return json.dumps(build_schema(record_type, model), ensure_ascii=False, separators=(",", ":"), sort_keys=True) + "\n"
...
def sync_schemas(output_dir: Path, *, check: bool = False) -> list[str]:
    ...
    problems.extend(
        f"unexpected: {filename}" for filename in sorted(set(existing) - set(expected))
    )
    return problems


def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Generate Osoznanie Protocol JSON Schemas.")
